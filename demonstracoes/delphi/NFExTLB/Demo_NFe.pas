...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, ShellAPI, IniFiles, spdNFeDataSets, spdNFeType,
  ComCtrls, XPMan, spdNFeUtils, jpeg, NFeX_TLB,
...
begin
  ...
  cbCertificado.Items.Add(spdNFe.ListarCertificados('|'));
end;
//-----------------------------------------------------------------------------
...
begin
  spdNFe.NomeCertificado := cbCertificado.Text;
...
//    mmXml.Text := spdNFe.ConverterLoteParaXml(_XML,lkRec,'pl_009');
//    edtID.Text := obterNroResultado(mmXml.Text,'"NF','" versao');
//  end
//  else
//  begin
//    mmXml.Text := spdNFe.ConverterLoteParaXml(mmXml.Text,lkRec,'pl_009');
//    edtID.Text := obterNroResultado(mmXml.Text,'"NF','" versao');
//  end;
end;
...
begin
  ...
  begin
    _XML := dlgOpen.FileName;
    mmXml.Lines.LoadFromFile(_XML);
    mmXml.Text := spdNFe.ConverterLoteParaXml(_XML,lkTx2,'pl_009');
  end;
...
var
  PDFaux, XMLaux, email: string;
begin
  PDFaux := InputBox('Pergunta', 'Arquivo PDF', 'D:\Temp\Danfe.pdf');
  XMLaux := InputBox('Pergunta', 'Arquivo XML', 'D:\Temp\NF.XML');
  email  := InputBox('Pergunta', 'Email Destinatário', 'joao.costa@tecnospeed.com.br');

  spdNFe.EmailDestinatario := email;
